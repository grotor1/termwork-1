...
public class AnimeListServlet extends AbstractServlet {
    @Override
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        AnimeList animes = new AnimeList(DevPostgressConnection.getInstance());
        List<HashMap<String, String>> lsp;
        int pageCount;

        if (query != null) {
            pageCount = animes.getPageCount(query);
            lsp = animes.getAnimePage(page, query);
        } else {
            pageCount = animes.getPageCount();
            lsp = animes.getAnimePage(page);
        }
        ...
        root.put("animes", lsp.toArray());

        Template tmpl = FreemarkerConfig.getInstance().getTemplate("animes.ftl");
        try {
